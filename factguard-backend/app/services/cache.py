...
async def get_cached_analysis(claim_hash: str) -> dict | None:
    try:
        ...
        if client is None:
            return None
        data = await client.get(f"factguard:claim:{claim_hash}")
...
async def set_progress(job_id: str, step: str) -> None:
    try:
        ...
        if client is None:
            return
        await client.setex(f"factguard:progress:{job_id}", 300, step)
    except Exception:
        pass
...
async def get_progress(job_id: str) -> str | None:
    try:
        ...
        if client is None:
            return None
        data = await client.get(f"factguard:progress:{job_id}")
...
async def get_cached_history() -> list[dict] | None:
    try:
        ...
        if client is None:
            return None
        data = await client.lrange(HISTORY_CACHE_KEY, 0, -1)
...
async def get_job_query(job_id: str) -> str | None:
    try:
        ...
        if client is None:
            return None
        data = await client.get(f"factguard:query:{job_id}")
        return data if data else None
    except Exception as e:
...
async def get_job_result(job_id: str) -> dict | None:
    try:
        ...
        if client is None:
            return None
        data = await client.get(f"factguard:job_result:{job_id}")
        return json.loads(data) if data else None
    except Exception as e:
