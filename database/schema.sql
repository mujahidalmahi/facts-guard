...
raw_json JSONB, -- full Claude response stored for debug
created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
ON claims(claim_hash); -- cache dedup lookup
CREATE INDEX IF NOT EXISTS idx_claims_status
ON claims(status); -- poll queue filtering
CREATE INDEX IF NOT EXISTS idx_results_claim
ON results(claim_id); -- join speed
CREATE INDEX IF NOT EXISTS idx_sources_result
ON sources(result_id); -- join speed
CREATE INDEX IF NOT EXISTS idx_claims_created
ON claims(created_at DESC); -- history page sort